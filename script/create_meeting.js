...
modalAgreeBtn.addEventListener(
    "click",
...
async function getPatientName(thisPatientId) {
    await fetch("http://localhost:3000/patients?id=" + thisPatientId)
        ...
        .then((data) => {
            modalNameInput.value = data[0].name;
        })
...
async function getDoctorsList() {
    await fetch("http://localhost:3000/doctors")
        ...
        .then((data) => {
            data.forEach((object) => selectDoctorListHTML(object));
        })
        .catch((error) => console.log(error));
}
...
async function getMeetings() {
    await fetch(
        "http://localhost:3000/meetings?_expand=doctor&patientId=" + patientId
    )
        .then((result) => {
            return result.json();
        })
...
async function showID() {
    let doctorId = await fetch(
        "http://localhost:3000/doctors?name=" + modalSelectDoctor.value
    )
        .then((result) => {
            return result.json();
        })
...
let sendRequestBtns = document.getElementsByClassName(
    "doctor-dashbord__table_btn"
);
